...
var (
	...
	nameHover    = nameBase.Foreground(lipgloss.Color("189"))
...
/____/____/_/ /_/\___/_/ .___/ 
	...
	rainbow = []lipgloss.Color{
		...
		"190", "154", "118", "82", "46", "47", "48", "49", "50", "51",
		"45", "39", "33", "27", "21", "20", "19", "18", "17",
		"53", "89", "125", "161", "197",
	}

	pulseColors = []lipgloss.Color{
		"239", "240", "241", "242", "243", "242", "241", "240",
	}

	actPulseColors = []lipgloss.Color{
		"196", "160", "124", "160",
	}

	dialogPulseColors = []lipgloss.Color{
		"196", "124", "88", "124",
	}

	hoverGlow = lipgloss.Color("102")
...
func (m model) rainbowColor(step int64) lipgloss.Color {
	return rainbow[(m.frame/step)%int64(len(rainbow))]
}

func (m model) pulseColor(step int64, colors []lipgloss.Color) lipgloss.Color {
	return colors[(m.frame/step)%int64(len(colors))]
}

func (m model) calcHover(y int) int {
...
func (m model) handleUnlockKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		if err != nil {
			...
			m.err = "Wrong password or corrupted data"
...
func (m model) animatedBanner() string {
	c := m.rainbowColor(4)
	return lipgloss.NewStyle().Foreground(c).Render(bannerArt) + "\n"
}
...
func (m model) unlockView() string {
	...
	b.WriteString(m.animatedBanner())
	b.WriteString("\n")

	if m.isFirstRun {
		b.WriteString(headingStyle.Render("Create Master Password"))
	} else {
		b.WriteString(headingStyle.Render("Unlock"))
	}
	b.WriteString("\n\n")
	b.WriteString(m.unlockInput.View())
	b.WriteString("\n\n")
	b.WriteString(dimStyle.Render("[Enter] Confirm  [Esc] Quit"))
...
func (m model) listView() string {
	...
	if len(m.profiles) == 0 {
		dotIdx := int((m.frame / 15) % 4)
		b.WriteString(headingStyle.Render("  " + dots[dotIdx]))
		b.WriteByte('\n')
		b.WriteString(dimStyle.Render("  No profiles yet. Press 'a' to add one."))
	} else {
		maxW := m.width - 6
		if maxW < 30 {
			maxW = 30
		...
		for i, p := range m.profiles {
			sel := i == m.cursor
			hov := i == m.hoverIdx && !sel

			borderCol := borderGray
			nameSty := nameWhite

			if sel {
				borderCol = m.pulseColor(6, pulseColors)
				if borderCol == lipgloss.Color("239") {
					borderCol = red
				}
				if borderCol == lipgloss.Color("240") || borderCol == lipgloss.Color("241") {
					borderCol = lipgloss.Color("124")
				}
				nameSty = nameRed
			} else if hov {
				borderCol = hoverGlow
			...
			if sel && m.state == stateActions {
				...
				for ai, a := range actions {
					if ai == m.actionCursor {
						ac := m.pulseColor(8, actPulseColors)
						rendered[ai] = lipgloss.NewStyle().Bold(true).Foreground(ac).Render(a)
					} else {
						rendered[ai] = a
					}
				}
...
func (m model) addView() string {
	...
	stepLabel := ""
	if m.addStep <= 4 {
		stepLabel = "Step " + strconv.Itoa(m.addStep+1) + "/5: " + stepLabels[m.addStep]
	} else {
		stepLabel = "Review & Save"
	}
	b.WriteString(headingStyle.Render(stepLabel))
	b.WriteString("\n\n")
	...
	dCol := m.pulseColor(10, dialogPulseColors)
	dStyle := dialogStyle.BorderForeground(dCol)

	if m.addStep <= 4 {
		content := m.addInput.View() + "\n\n"
		content += dimStyle.Render("[Enter] Next  [Esc] Cancel")
		b.WriteString(dStyle.Width(maxW).Render(content))
	} else {
...
func (m model) editView() string {
	...
	for i, f := range fields {
		if i == m.editField && m.state == stateEdit {
			ac := lipgloss.NewStyle().Bold(true).Foreground(m.pulseColor(8, actPulseColors)).Render(">")
	...
	dCol := m.pulseColor(10, dialogPulseColors)
	dStyle := dialogStyle.BorderForeground(dCol)
	b.WriteString(dStyle.Width(maxW).Render(content.String()))
